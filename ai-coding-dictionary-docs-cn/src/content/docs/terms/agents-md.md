...
A file in the [environment](/terms/environment) that the [harness](/terms/harness) loads into the [context window](/terms/context-window) at [session](/terms/session) start — the project's standing brief to the [agent](/terms/agent). Cross-harness convention; some harnesses also have their own variant (Claude Code's is CLAUDE.md).

Because it loads automatically, it's one way to avoid repeating yourself across sessions. The [model](/terms/model) is [stateless](/terms/stateless) — a correction you give in one session is gone in the next, and you end up telling every fresh session that the project uses pnpm, that tests run with a particular flag, that a directory is generated and shouldn't be touched. When you've corrected the agent for the same thing twice, that correction is a candidate line for AGENTS.md.
...
_Avoid:_ using AGENTS.md for content that should be [progressively disclosed](/terms/progressive-disclosure) — anything in it pays a [token](/terms/token) cost every [turn](/terms/turn), in every session, whether or not that session needs it. A style guide can go behind a [skill](/terms/skill) or a [context pointer](/terms/context-pointer) instead; keep AGENTS.md for the lines that apply everywhere.
