...
> Beta Remote Orca Servers are beta. Keep the server and client on a private network path you control, such as the same Tailscale tailnet or LAN.
...
4. For **Connection address**, select the Tailscale address. It usually looks like `100.x.y.z`.
...
> Keep the access link private The pairing URL grants access to this Orca runtime. Treat it like a password and send it only to the client you intend to pair.
...
2. Click **Add Server**.
3. Enter a recognizable name, such as `Remote Server`.
