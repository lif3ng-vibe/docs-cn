...
Orca 会用生效的 OpenSSH known_hosts 文件与它此前保存的密钥来校验内建 SSH 连接。已匹配的主机静默连接。默认策略下，Orca 在首次接触时接受并记住主机、显示其指纹；`StrictHostKeyChecking yes` 拒绝未知主机，`no` / `off` 则接受但不保存信任记录。密钥发生变化、被吊销或类型意外不同，会在 Orca 索要密码或密钥口令**之前**被拒绝。如果你是有意重建或重新开通了这台主机，先核实新指纹，再按错误提示里的恢复路径处理。遇到 known_hosts 不匹配时，Orca 会给出对应的 `ssh-keygen -R` 命令（需要时含方括号包裹的主机与端口）。
