...
- **按仓库的捕获排除。** 就近标记的 `[capture]` `ignore_paths` 策略在匹配的已识别文件工具事件到达本地暂存（spool）或服务器之前丢弃它们。见[捕获排除策略参考](/marker-file/#捕获排除-capture-exclusions)。
- **可选的按操作者记忆槽位。** 共享服务器上，`[slots] per_user = true` 把引擎写入的 `_slots/` 上下文保存在由已认证操作者派生的有界命名空间里。会话简报与整编提示词收到共享槽位加上调用者自己的；精确的 wiki 读取与检索仍是全项目范围，所以这是上下文注入隔离而非 RBAC。见[多用户运行](/users/#按操作者的记忆槽位-per-operator-memory-slots)。
