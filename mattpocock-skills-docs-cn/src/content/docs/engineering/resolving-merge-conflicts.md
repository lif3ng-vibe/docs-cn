...
| Merge finished, something now misbehaves for reasons you can't see | [diagnosing-bugs](/engineering/diagnosing-bugs) |
...
A reach-for-it-anytime standalone with no dependencies on any other skill: it starts when git stalls and ends when the tree is clean and committed. Its only real neighbour is [diagnosing-bugs](/engineering/diagnosing-bugs), which takes over at the point where a merge resolved cleanly but the merged code misbehaves: a diagnosis problem, not a conflict one. It sits off the main idea-to-ship flow entirely, so [ask-matt](/engineering/ask-matt) is the map for what runs before and after it.
