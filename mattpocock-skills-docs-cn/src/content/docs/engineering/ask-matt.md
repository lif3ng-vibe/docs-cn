...
| Bugs and requests arriving from other people | The [triage](/engineering/triage) on-ramp, and why [tickets](https://www.aihero.dev/ai-coding-dictionary/ticket) you generated yourself don't belong on it |
| Two skills that look interchangeable | The line between them, and it is usually one concrete test rather than a matter of taste. [grill-me](/productivity/grill-me) or [grill-with-docs](/engineering/grill-with-docs) turns on whether you are in a working directory; [grill-with-docs](/engineering/grill-with-docs) or [wayfinder](/engineering/wayfinder) turns on whether the effort fits one session |
...
The tracker-dependent routes (triage, `to-spec`, `to-tickets`, `implement`) assume [setup-matt-pocock-skills](/engineering/setup-matt-pocock-skills) has already configured an issue tracker in the repo. The router will happily recommend them before that has happened.
...
| **[handoff](/productivity/handoff)** | Something has to travel: a new [harness](https://www.aihero.dev/ai-coding-dictionary/harness), a new directory, a colleague, a side task forked mid-phase |
...
`ask-matt` is a **standalone router** that sits over the whole set. It is never a step in a chain; it points into every chain, and it is the node the other docs pages link back to so none of them has to redraw the graph. From here you most often land on [grill-with-docs](/engineering/grill-with-docs), the head of the main flow, or [triage](/engineering/triage), the on-ramp for work that arrived rather than work you started.
