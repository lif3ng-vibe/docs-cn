...
| Nothing is written yet and you want it written test-first | [tdd](/engineering/tdd) |
| A whole spec needs building, review included | [implement](/engineering/implement), which calls this skill itself |
| The whole codebase has drifted, not one diff | [improve-codebase-architecture](/engineering/improve-codebase-architecture) |
| Something is broken and you do not know why | [diagnosing-bugs](/engineering/diagnosing-bugs) |
...
Step 1 depends on `docs/agents/issue-tracker.md`, which [setup-matt-pocock-skills](/engineering/setup-matt-pocock-skills) writes. Without it the axis still works if you hand it a path. With no spec at all, the Spec sub-agent is skipped and the report says "no spec available" rather than inventing requirements.
...
- [implement](/engineering/implement) is the closest neighbour: it drives the build and calls this skill as its own closing review before committing.
- [to-spec](/engineering/to-spec) and [to-tickets](/engineering/to-tickets) produce the document the Spec axis checks against; a vague spec makes that axis vague.
- [improve-codebase-architecture](/engineering/improve-codebase-architecture) is the whole-codebase counterpart: this skill only ever looks at one diff.

[ask-matt](/engineering/ask-matt) routes across the whole set when you are unsure which skill the situation wants.
