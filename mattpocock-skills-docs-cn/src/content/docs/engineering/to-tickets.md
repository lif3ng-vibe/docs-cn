...
| The whole change fits in one context window | [implement](/engineering/implement), skip the tickets |
| Nothing is decided yet | [grill-with-docs](/engineering/grill-with-docs), then [to-spec](/engineering/to-spec) |
| A [wayfinder](/engineering/wayfinder) map has cleared | [to-spec](/engineering/to-spec) first, to collapse the map, then `/to-tickets` |

Tickets that `to-tickets` produced are agent-ready by construction. Don't run [triage](/engineering/triage) over them. Triage is for work that arrived from someone else.

## Prerequisites

`to-tickets` publishes into a tracker, so [setup-matt-pocock-skills](/engineering/setup-matt-pocock-skills) must have configured one for this repo, along with the triage-label vocabulary. Either kind works: a real tracker like GitHub or Linear, or local markdown files under `.scratch/`, which is supported out of the box.
...
The skill stops at the artifact, and there is no auto-dispatch mode. Dispatch is manual: look at the board, count the tickets with no open blockers, and open that many agent sessions. One ticket per fresh context, cleared between them. Be aware that [implement](/engineering/implement) does not reliably close or check off the ticket when it finishes, on GitHub or in local markdown, so the ticket's state is yours to update.
...
Upstream is [to-spec](/engineering/to-spec), which hands it a settled spec to slice against; keep both in one unbroken context window. Downstream is [implement](/engineering/implement), which builds one ticket per fresh session, driving [tdd](/engineering/tdd) for the tests and closing with [code-review](/engineering/code-review). When you're unsure which skill or flow fits, [ask-matt](/engineering/ask-matt) routes you.
