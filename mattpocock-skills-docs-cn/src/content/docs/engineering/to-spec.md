...
| --- | --- |
| You haven't decided anything yet | [grill-with-docs](/engineering/grill-with-docs) first |
| Decided, and the work fits one [context window](https://www.aihero.dev/ai-coding-dictionary/context-window) | [implement](/engineering/implement): skip the spec |
| Decided, and the work spans several sessions | `/to-spec`, then [to-tickets](/engineering/to-tickets) |
| A [wayfinder](/engineering/wayfinder) map has cleared | `/to-spec #<map_issue>` |

## Prerequisites

`to-spec` publishes the spec as an issue, so [setup-matt-pocock-skills](/engineering/setup-matt-pocock-skills) must have configured a tracker and the triage-label vocabulary for this repo first. Either kind works: a real tracker like GitHub, or local markdown files under `.scratch/`, which is supported out of the box.
...
The main map issue: `/to-spec #<map_issue>`, not the individual decision tickets. [wayfinder](/engineering/wayfinder) produces decisions rather than deliverables, scattered across a map; `to-spec` is the step that collapses them into one buildable document. Looping the map straight into `/implement` throws that collapse away.
...
Less well, and this is a known limitation. The template leans hard on user stories, which is the wrong shape for architectural work: you end up writing stories nobody asked for around decisions that are really about interfaces and invariants. Lean on the implementation-decisions and testing-decisions sections instead, and let the durable architectural calls land as ADRs via [grill-with-docs](/engineering/grill-with-docs) rather than trying to make the spec carry them.
...
Its neighbours upstream are [grill-with-docs](/engineering/grill-with-docs), which does the deciding this skill only records, and [wayfinder](/engineering/wayfinder), whose finished map merges onto the chain right here. Downstream, [to-tickets](/engineering/to-tickets) cuts the spec into tracer-bullet tickets for [implement](/engineering/implement) to build. When you're unsure which skill or flow fits, [ask-matt](/engineering/ask-matt) routes you.
