...
| The behaviour isn't pinned down yet | [to-spec](/engineering/to-spec), which also agrees the test seams before any code is written |
| The question is really the shape of the interface, not the tests | [codebase-design](/engineering/codebase-design) |
| You have a [spec](https://www.aihero.dev/ai-coding-dictionary/spec) or [tickets](https://www.aihero.dev/ai-coding-dictionary/ticket) and want the whole build run for you | [implement](/engineering/implement), which drives `tdd` per ticket |
...
Into [codebase-design](/engineering/codebase-design) in v1.0, generalised so several skills share one vocabulary. `refactoring.md` left at the same time; refactoring is now [code-review](/engineering/code-review)'s job, and that skill carries the Fowler smell baseline.
...
[to-spec](/engineering/to-spec) agrees the test seams up front, [implement](/engineering/implement) drives `tdd` per ticket, and [code-review](/engineering/code-review) checks afterwards that only the agreed seams were used, and owns the refactoring `tdd` no longer does. Its other neighbour is [codebase-design](/engineering/codebase-design), the shared source of the seam and deep-module vocabulary `tdd` speaks. You can also reach for it on its own, whenever there is a concrete behaviour to build and no full spec in play. When you are unsure which skill fits your situation, [ask-matt](/engineering/ask-matt) routes you.
